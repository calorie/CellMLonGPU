...
public class MpiMainFuncGenerator extends MpiProgramGenerator {
    ...
    // ========================================================
    /*-----プログラム構文取得メソッド-----*/
    public SyntaxFunction getSyntaxMainFunction() throws MathException,
        ...
        Math_ci pLoopIndexStartVar = createIntVal(pSynMainFunc, START, 0, 0,
                false);
        ...
        /* ループ変数インスタンス生成 */
        Math_ci pLoopIndexVar2 = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, COMPROG_LOOP_INDEX_NAME2);
        ...
        // mpi用変数生成
        /*
         * Assign all nodes result double* all_xo;
         */
        Math_ci pMpiAllXoIndexVar = createDoubleVal(pSynMainFunc, ALL_XO, 1, 0,
                false);

        /*
         * Assign all nodes result double* tmp_xo;
         */
        Math_ci pMpiTmpXoIndexVar = createDoubleVal(pSynMainFunc, TMP_XO, 1, 0,
                false);

        /*
         * end time double end_time = 400.0;
         */
        Math_ci pMpiEndTimeIndexVar = createDoubleVal(pSynMainFunc, END_TIME,
                0, END_TIME_NUM, false);

        /*
         * Number of all nodes int comm_size;
         */
        Math_ci pMpiCommSizeIndexVar = createIntVal(pSynMainFunc, COMM_SIZE, 0,
                0, false);

        /*
         * Node number(Master:0, Worker:0~comm_size-1) int node;
         */
        Math_ci pMpiNodeIndexVar = createIntVal(pSynMainFunc, NODE, 0, 0, false);

        /*
         * Tag for MPI_Send and MPI_Recv int tag=0;
         */
        Math_ci pMpiTagIndexVar = createIntVal(pSynMainFunc, TAG, 0, TAG_NUM,
                true);

        /*
         * Node number of a worker int worker;
         */
        Math_ci pMpiWorkerIndexVar = createIntVal(pSynMainFunc, WORKER, 0, 0,
                false);

        /*
         * MPI_Recv status MPI_Status recv_status;
         */
        Math_ci pMpiRecvStatusIndexVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, RECV_STATUS);

        /* 変数を関数に宣言追加 */
        {
            SyntaxDataType pSynTypeInt = new SyntaxDataType(
                    eDataType.DT_MPISTA, 0);
            SyntaxDeclaration pDecVar = new SyntaxDeclaration(pSynTypeInt,
                    pMpiRecvStatusIndexVar);

            pSynMainFunc.addDeclaration(pDecVar);
        }

        /*
         * Master data size int __MASTER_DATA_NUM;
         */
        Math_ci pMpiMasterDataNumIndexVar = createIntVal(pSynMainFunc,
                MASTER_DATA_NUM, 0, 0, false);

        /*
         * Worker data size int __WORKER_DATA_NUM;
         */
        Math_ci pMpiWorkerDataNumIndexVar = createIntVal(pSynMainFunc,
                WORKER_DATA_NUM, 0, 0, false);

        /*
         * MPI initilize MPI community size Get node number of this node
         */
        ...
        if (m_isTestGenerate)
            pSynMainFunc.addStatement(m_testInitFunc.callFunction(
                    (Math_ci) MathFactory.createOperand(eMathOperand.MOPD_CI,
                            PROG_VAR_STR_ARGV), pMpiNodeIndexVar));

        // create if for Decide data size
        Math_cn commSizeMax = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, COMM_SIZE_INDEX);
        Math_lt pMathLt = (Math_lt) MathFactory
                .createOperator(eMathOperator.MOP_LT);
        // create cond
        MathFactor pSynIfCond = createCondition(pMpiCommSizeIndexVar,
                commSizeMax, pMathLt);
        // create if
        ...
        // if's element
        Math_divide pMathDivide = createDivide(m_pDefinedDataSizeVar,
                pMpiCommSizeIndexVar);
        Math_minus pMathMinus = createMinus(m_pDefinedDataSizeVar,
        ...
        // else's elsemet
        Math_divide pMathDivideForElse = createDivide(m_pDefinedDataSizeVar,
                pMpiCommSizeIndexVar);
        Math_cn commSizeMinusNum = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, COMM_SIZE_MINUS_NUM);
        Math_minus pMathMinusForElse = createMinus(pMpiCommSizeIndexVar,
                commSizeMinusNum);
        Math_times pMathTimesForElse = createTimes(pMpiWorkerDataNumIndexVar,
                pMathMinusForElse);
        pMathMinusForElse = createMinus(m_pDefinedDataSizeVar,
                pMathTimesForElse);
        // add else
        ...
        // start's element
        Math_cn nodeMinusNum = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, "1");
        Math_minus pMathMinusForStart = createMinus(pMpiNodeIndexVar,
                nodeMinusNum);
        Math_times pMathTimesForStart = createTimes(pMpiWorkerDataNumIndexVar,
                pMathMinusForStart);
        Math_plus pMathPlusForStart = createPlus(pMathTimesForStart,
                pMpiMasterDataNumIndexVar);
        // add main
        createAssign(pSynMainFunc, pLoopIndexStartVar, pMathPlusForStart);

        // end's element
        Math_plus pMathPlusForEnd = createPlus(pLoopIndexStartVar,
        ...
        Math_cn masterNodeNum = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, MASTER_NODE_NUM);
        // create condition
        MathFactor pSynIfForMasterNodeCond = createCondition(pMpiNodeIndexVar,
                masterNodeNum, pMathEq);
        // add cond to if
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecDiffVar().size(); i++) {
            ...
            /* データ数を表す数式を生成 */
            Math_times pMathTimesForMaster = (Math_times) MathFactory
                    .createOperator(eMathOperator.MOP_TIMES);
            Math_times pMathTimesForSlave = (Math_times) MathFactory
                    .createOperator(eMathOperator.MOP_TIMES);
            ...
            /* 宣言の追加 */
            pSynIfForMasterNode.addStatement(createMalloc(m_pTecMLAnalyzer
                    .getM_vecDiffVar().get(i), pMathTimesForMaster));
            pSynIfForSlaveNode.addStatement(createMalloc(m_pTecMLAnalyzer
                    .getM_vecDiffVar().get(i), pMathTimesForSlave));
        }
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecArithVar().size(); i++) {
            ...
            /* 宣言の追加 */
            pSynIfForMasterNode.addStatement(createMalloc(m_pTecMLAnalyzer
                    .getM_vecArithVar().get(i), pMathTimesForMaster));
            pSynIfForSlaveNode.addStatement(createMalloc(m_pTecMLAnalyzer
                    .getM_vecArithVar().get(i), pMathTimesForSlave));
        }
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecDerivativeVar().size(); i++) {
            ...
            /* 宣言の追加 */
            pSynIfForMasterNode.addStatement(createMalloc(m_pTecMLAnalyzer
                    .getM_vecDerivativeVar().get(i), pMathTimesForMaster));
            pSynIfForSlaveNode.addStatement(createMalloc(m_pTecMLAnalyzer
                    .getM_vecDerivativeVar().get(i), pMathTimesForSlave));
        }
        ...
        /* 内側ループ構文生成・追加 */
        SyntaxControl pSynFor2ForMaster = createSyntaxDataNumLoop(
                pMpiMasterDataNumIndexVar, pLoopIndexVar);
        SyntaxControl pSynFor2ForSlave = createLoop(pLoopIndexStartVar,
                pLoopIndexEndVar, pLoopIndexVar);
        ...
        // ----------------------------------------------
        /* 数式を生成し，取得 */
        Vector<SyntaxExpression> vecExpressionsForMaster = this
                .createExpressions(pMpiMasterDataNumIndexVar);
        Vector<SyntaxExpression> vecExpressionsForSlave = this
                .createExpressions(pMpiWorkerDataNumIndexVar);
        ...
        for (int i = 0; i < m_pCellMLAnalyzer.getM_vecDiffVar().size(); i++) {
            // TODO
            Math_ci pMpiAllXoIndexVartes = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, ALL_XO);
            Math_cn allxoLoopVar = (Math_cn) MathFactory.createOperand(
                    eMathOperand.MOPD_CN, "" + i);

            // all_xo's []
            Math_times tmpTimes = createTimes(allxoLoopVar,
            ...
            /* 数式の複製を取得 */
            // MathExpression pMathExp =
            // m_pCellMLAnalyzer.getM_vecDiffExpression().get(i);
            /* 左辺式取得 */
            // MathExpression pLeftExp = pMathExp.getLeftExpression();
            // TODO
            Math_ci pMpixo = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, MPI_XO);
            Math_times tmpTimes2 = createTimes(allxoLoopVar,
                    pMpiMasterDataNumIndexVar);
        ...
        // create recv loop
        Math_cn workerStartNum = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, WORKER_START_NUM);
        SyntaxControl recvLoop = createLoop(workerStartNum,
                pMpiCommSizeIndexVar, pMpiWorkerIndexVar);
        pSynFor1ForMaster.addStatement(recvLoop);

        Math_ci pMpiDouble = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, MPI_DOUBLE);
        // MPI_Recv
        // TODO
        Math_ci recvSize = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI,
                createTimes(
                        pMpiWorkerDataNumIndexVar,
                        (Math_cn) MathFactory.createOperand(
                                eMathOperand.MOPD_CN, ""
                                        + m_pCellMLAnalyzer.getM_vecDiffVar()
                                                .size())).toLegalString());
        pMpiTmpXoIndexVar.setArrayIndexToBack((Math_cn) MathFactory
                .createOperand(eMathOperand.MOPD_CN, "0"));
        pMpiTmpXoIndexVar.setPointerNum(-1);
        pMpiRecvStatusIndexVar.setPointerNum(-1);
        SyntaxCallFunction pMpiRecv = createMpiRecv(pMpiTmpXoIndexVar,
                recvSize, pMpiDouble, pMpiWorkerIndexVar, pMpiTagIndexVar,
                pMpiRecvStatusIndexVar);
        recvLoop.addStatement(pMpiRecv);

        // __j = 0;
        createAssign(recvLoop, pLoopIndexVar2,
                (Math_cn) MathFactory.createOperand(eMathOperand.MOPD_CN, "0"));

        // create for loop
        SyntaxControl allxoLoop = createLoop(
                (Math_cn) MathFactory.createOperand(eMathOperand.MOPD_CN, "0"),
                        ...
                        createTimes(
                                createMinus(pMpiWorkerIndexVar,
                                        (Math_cn) MathFactory.createOperand(
                                                eMathOperand.MOPD_CN, "1")),
                                pMpiWorkerDataNumIndexVar),
                        pMpiMasterDataNumIndexVar), pLoopIndexVar2);
        ...
        createAssign(allxoLoop, (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, pMpiAllXoIndexVar.toLegalString()),
                (Math_ci) MathFactory.createOperand(eMathOperand.MOPD_CI,
                        pMpiTmpXoIndexVar.toLegalString()));

        // create if for set allxoo
        pMathEq = (Math_eq) MathFactory.createOperator(eMathOperator.MOP_EQ);
        // create if
        SyntaxControl pSynAllxoIf = createIf(createCondition(
                createRemainder(
                        createPlus(pLoopIndexVar,
                                (Math_cn) MathFactory.createOperand(
                                        eMathOperand.MOPD_CN, "1")),
                        pMpiWorkerDataNumIndexVar),
                (Math_cn) MathFactory.createOperand(eMathOperand.MOPD_CN, "0"),
        ...
        createAssign(
                ...
                pLoopIndexVar2,
                createPlus(
                        createMinus(m_pDefinedDataSizeVar,
                                pMpiWorkerDataNumIndexVar), pLoopIndexVar2));

        // j++
        createAssign(
                allxoLoop,
                pLoopIndexVar2,
                createPlus(pLoopIndexVar2, (Math_cn) MathFactory.createOperand(
                        eMathOperand.MOPD_CN, "1")));
        ...
        if (m_isTestGenerate) {
            ...
            for (int i = 0; i < m_pCellMLAnalyzer.getM_vecDiffVar().size(); i++) {
                pMpiAllXoIndexVar.setArrayIndexToBack(createTimes(
                        m_pDefinedDataSizeVar, (Math_cn) MathFactory
                                .createOperand(eMathOperand.MOPD_CN, "" + i)));
                pSynFor1ForMaster.addStatement(m_testFunc.callFunction(
                        (Math_cn) MathFactory.createOperand(
                                eMathOperand.MOPD_CN, "\"" + ALLXO_TEST_NAME
                                        + i + "\""), (Math_ci) MathFactory
                                .createOperand(eMathOperand.MOPD_CI,
                                        pMpiAllXoIndexVar.toLegalString()),
                        (Math_cn) MathFactory.createOperand(
                                eMathOperand.MOPD_CN, TEST_DIFF)));
            }
        }
        ...
        Math_ci sendSize = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI,
                createTimes(
                        pMpiWorkerDataNumIndexVar,
                        (Math_cn) MathFactory.createOperand(
                                eMathOperand.MOPD_CN, ""
                                        + m_pCellMLAnalyzer.getM_vecDiffVar()
                                                .size())).toLegalString());
        SyntaxCallFunction pMpiSend = createMpiSend(pMpixo, sendSize,
                pMpiDouble, m_pDefinedDataSizeVar2, pMpiTagIndexVar);
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecDiffVar().size(); i++) {
            /* 宣言の追加 */
            pSynIfForMasterNode.addStatement(createFree(m_pTecMLAnalyzer
                    .getM_vecDiffVar().get(i)));
            pSynIfForSlaveNode.addStatement(createFree(m_pTecMLAnalyzer
                    .getM_vecDiffVar().get(i)));
        }
        /* 一時変数のfree */
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecArithVar().size(); i++) {
     ...
     */
    public void mpiValCreateFunction(SyntaxFunction pSynMainFunc)
            throws MathException {
        /*
         * Assign all nodes result double* all_xo;
         */
