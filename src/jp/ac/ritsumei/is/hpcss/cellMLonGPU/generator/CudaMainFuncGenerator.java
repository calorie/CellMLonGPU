...
/**
 * CUDAメイン関数構文生成クラス CudaProgramGeneratorクラスからメイン関数生成部を切り離したクラス
 */
public class CudaMainFuncGenerator extends CudaProgramGenerator {
    ...
    public SyntaxFunction getSyntaxMainFunction() throws MathException,
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecDiffVar().size(); i++) {
            ...
            /* 宣言用変数の生成 */
            Math_ci pDecHostVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, CUPROG_HOST_VARIABLES_PREFIX
                            + m_pTecMLAnalyzer.getM_vecDiffVar().get(i)
                                    .toLegalString());
            Math_ci pDecDevVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, CUPROG_DEVICE_VARIABLES_PREFIX
                            + m_pTecMLAnalyzer.getM_vecDiffVar().get(i)
                                    .toLegalString());

            /* 宣言の生成 */
            SyntaxDeclaration pSynHostVarDec = new SyntaxDeclaration(
                    pSynTypePDouble, pDecHostVar);
            SyntaxDeclaration pSynDevVarDec = new SyntaxDeclaration(
                    pSynTypePDouble, pDecDevVar);
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecDerivativeVar().size(); i++) {
            ...
            Math_ci pDecHostVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, CUPROG_HOST_VARIABLES_PREFIX
                            + m_pTecMLAnalyzer.getM_vecDerivativeVar().get(i)
                                    .toLegalString());
            Math_ci pDecDevVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, CUPROG_DEVICE_VARIABLES_PREFIX
                            + m_pTecMLAnalyzer.getM_vecDerivativeVar().get(i)
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecArithVar().size(); i++) {
            ...
            Math_ci pDecHostVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, CUPROG_HOST_VARIABLES_PREFIX
                            + m_pTecMLAnalyzer.getM_vecArithVar().get(i)
                                    .toLegalString());
            Math_ci pDecDevVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, CUPROG_DEVICE_VARIABLES_PREFIX
                            + m_pTecMLAnalyzer.getM_vecArithVar().get(i)
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecConstVar().size(); i++) {
            ...
            Math_ci pDecHostVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, CUPROG_HOST_VARIABLES_PREFIX
                            + m_pTecMLAnalyzer.getM_vecConstVar().get(i)
                                    .toLegalString());
            Math_ci pDecDevVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, CUPROG_DEVICE_VARIABLES_PREFIX
                            + m_pTecMLAnalyzer.getM_vecConstVar().get(i)
        ...
        /* グリッド・ブロック変数宣言を生成 */
        Math_ci pBlockVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_BLOCK_VAR_NAME);
        Math_ci pGridVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_GRID_VAR_NAME);
        ...
        for (int i = 0; i < m_vecSynHostTimeVarDec.size(); i++) {
            ...
            /* コピー先変数インスタンス生成 */
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynHostTimeVarDec
                    .get(i)).getDeclaredVariable().createCopy());

            /* 要素数を文字列化 */
            String strDataNumVar = String.valueOf(m_pCellMLAnalyzer
                    .getM_vecDiffVar().size());

            /* データ数計算式の構築 */
            Math_cn pDataNumVar = (Math_cn) MathFactory.createOperand(
                    eMathOperand.MOPD_CN, strDataNumVar);
            ...
            /* malloc文を追加 */
            pSynMainFunc.addStatement(this
                    .createMalloc(pTmpDstVar, pMathTimes1));
        }
        for (int i = 0; i < m_vecSynHostDiffVarDec.size(); i++) {

            /* コピー先変数インスタンス生成 */
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynHostDiffVarDec
                    .get(i)).getDeclaredVariable().createCopy());
        ...
        for (int i = 0; i < m_vecSynHostVarDec.size(); i++) {
            ...
            /* コピー先変数インスタンス生成 */
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynHostVarDec
                    .get(i)).getDeclaredVariable().createCopy());

            /* 要素数を文字列化 */
            String strDataNumVar = String.valueOf(m_pCellMLAnalyzer
                    .getM_vecArithVar().size());
        ...
        for (int i = 0; i < m_vecSynHostConstVarDec.size(); i++) {
            ...
            /* コピー先変数インスタンス生成 */
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynHostConstVarDec
                    .get(i)).getDeclaredVariable().createCopy());

            /* 要素数を文字列化 */
            String strDataNumVar = String.valueOf(m_pCellMLAnalyzer
                    .getM_vecConstVar().size());
            ...
            /* malloc文を追加 */
            pSynMainFunc.addStatement(this
                    .createMalloc(pTmpDstVar, pDataNumVar));
        }
        ...
        for (int i = 0; i < m_vecSynHostTimeVarDec.size(); i++) {
            ...
            /* memset文を追加 */
            pSynMainFunc.addStatement(this.createZeroMemset(pTmpDstVar,
                    pMathTimes1));
        }
        for (int i = 0; i < m_vecSynHostDiffVarDec.size(); i++) {
        ...
        for (int i = 0; i < m_vecSynDevTimeVarDec.size(); i++) {
            ...
            /* コピー先変数インスタンス生成 */
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevTimeVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            ...
            /* cudaMalloc文を追加 */
            pSynMainFunc.addStatement(this.createCudaMalloc(pTmpDstVar,
                    pMathTimes1));
        }
        for (int i = 0; i < m_vecSynDevDiffVarDec.size(); i++) {

            /* コピー先変数インスタンス生成 */
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevDiffVarDec
                    .get(i)).getDeclaredVariable().createCopy());
        ...
        for (int i = 0; i < m_vecSynDevVarDec.size(); i++) {
            ...
            /* コピー先変数インスタンス生成 */
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevVarDec
                    .get(i)).getDeclaredVariable().createCopy());
        ...
        for (int i = 0; i < m_vecSynDevConstVarDec.size(); i++) {
            ...
            /* コピー先変数インスタンス生成 */
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevConstVarDec
                    .get(i)).getDeclaredVariable().createCopy());
        ...
        for (int i = 0; i < m_vecSynHostTimeVarDec.size(); i++) {
            ...
            /* コピー元・コピー先変数インスタンス生成 */
            Math_ci pTmpSrcVar = (Math_ci) (((SyntaxDeclaration) m_vecSynHostTimeVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevTimeVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            ...
            /* cudaMemcpy文を追加 */
            pSynMainFunc.addStatement(this.createCudaMemcpyH2D(pTmpDstVar,
                    pTmpSrcVar, pMathTimes1));

        }
        for (int i = 0; i < m_vecSynHostDiffVarDec.size(); i++) {

            /* コピー元・コピー先変数インスタンス生成 */
            Math_ci pTmpSrcVar = (Math_ci) (((SyntaxDeclaration) m_vecSynHostDiffVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevDiffVarDec
                    .get(i)).getDeclaredVariable().createCopy());
        ...
        for (int i = 0; i < m_vecSynHostVarDec.size(); i++) {
            ...
            /* コピー元・コピー先変数インスタンス生成 */
            Math_ci pTmpSrcVar = (Math_ci) (((SyntaxDeclaration) m_vecSynHostVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevVarDec
                    .get(i)).getDeclaredVariable().createCopy());
        ...
        for (int i = 0; i < m_vecSynHostConstVarDec.size(); i++) {
            ...
            /* コピー元・コピー先変数インスタンス生成 */
            Math_ci pTmpSrcVar = (Math_ci) (((SyntaxDeclaration) m_vecSynHostConstVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            Math_ci pTmpDstVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevConstVarDec
                    .get(i)).getDeclaredVariable().createCopy());
        ...
        // ----------------------------------------------
        /* 初期カーネル呼び出しの追加 */
        SyntaxCallFunction pInitKernelCall = new SyntaxCallFunction(
                CudaInitKernelGenerator.CUPROG_INIT_KERNEL_NAME);
        ...
        /* 引数の追加 */
        pInitKernelCall
                .addArgFactor(m_pSynDevInputVarDec.getDeclaredVariable());
        ...
        // ----------------------------------------------
        /* 計算カーネル呼び出しの追加 */
        SyntaxCallFunction pKernelCall = new SyntaxCallFunction(
                CudaCalcKernelGenerator.CUPROG_CALC_KERNEL_NAME);
        pSynFor1.addStatement(pKernelCall);
        ...
        /* 計算カーネル引数追加 */
        for (int i = 0; i < m_vecSynDevTimeVarDec.size(); i++) {
            Math_ci pParamVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevTimeVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            pKernelCall.addArgFactor(pParamVar);
        }
        for (int i = 0; i < m_vecSynDevDiffVarDec.size(); i++) {
            Math_ci pParamVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevDiffVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            pKernelCall.addArgFactor(pParamVar);
        }
        for (int i = 0; i < m_vecSynDevVarDec.size(); i++) {
            Math_ci pParamVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            pKernelCall.addArgFactor(pParamVar);
        }
        for (int i = 0; i < m_vecSynDevConstVarDec.size(); i++) {
            Math_ci pParamVar = (Math_ci) (((SyntaxDeclaration) m_vecSynDevConstVarDec
                    .get(i)).getDeclaredVariable().createCopy());
            pKernelCall.addArgFactor(pParamVar);
        }
