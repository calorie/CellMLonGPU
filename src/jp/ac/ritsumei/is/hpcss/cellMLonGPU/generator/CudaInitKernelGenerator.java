...
/**
 * CUDA初期化カーネル構文生成クラス CudaProgramGeneratorクラスから初期化カーネル生成部を切り離したクラス
 */
public class CudaInitKernelGenerator extends CudaProgramGenerator {
    ...
    // ========================================================
    /*-----プログラム構文取得メソッド-----*/
    public SyntaxFunction getSyntaxInitKernel() throws MathException {
        // ----------------------------------------------
        // カーネル生成
        // ----------------------------------------------
        /* カーネル生成 */
        SyntaxFunction pSynInitKernel = this
                .createKernel(CUPROG_INIT_KERNEL_NAME);
        ...
        /* 入力出力変数の追加 */
        {
            ...
            Math_ci pDecVar = (Math_ci) MathFactory.createOperand(
                    eMathOperand.MOPD_CI, m_pTecMLAnalyzer.getM_pInputVar()
                            .toLegalString());

            /* 宣言の生成 */
            SyntaxDeclaration pSynInputVarDec = new SyntaxDeclaration(
