...
public class MpiProgramGenerator extends ProgramGenerator {
    ...
    public SyntaxProgram getSyntaxProgram() throws MathException,
        ...
        SyntaxPreprocessor pSynInclude4 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_INCLUDE_REL, "mpi.h");
        ...
        SyntaxPreprocessor pSynDefine2 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_DEFINE,
                m_pDefinedDataSizeVar2.toLegalString() + " " + strElementNum);
        pSynProgram.addPreprocessor(pSynDefine2);

        if (m_isTestGenerate) {
            TestFuncGenerator pTestFuncGenerator = new TestFuncGenerator(
                    m_pCellMLAnalyzer, m_pRelMLAnalyzer, m_pTecMLAnalyzer,
                    pSynProgram);
            pTestFuncGenerator.setTestInitFunc(pTestFuncGenerator
                    .getSyntaxTestInitFunction());
            pTestFuncGenerator.setTestFunc(pTestFuncGenerator
        ...
        // ----------------------------------------------
        /* メイン関数の生成 */
        MpiMainFuncGenerator pMpiMainFuncGenerator = new MpiMainFuncGenerator(
                m_pCellMLAnalyzer, m_pRelMLAnalyzer, m_pTecMLAnalyzer);
        pMpiMainFuncGenerator.setElementNum(m_unElementNum);
        pMpiMainFuncGenerator.setTimeParam(m_dStartTime, m_dEndTime,
                m_dDeltaTime);
        SyntaxFunction pSynMainFunc = pMpiMainFuncGenerator
    ...
    protected void initialize() throws MathException {
        ...
        m_pDefinedDataSizeVar2 = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, MASTER_PROCESS);
    }
    ...
    protected Vector<SyntaxExpression> createExpressions(
            Math_ci pMpiDataNumIndexVar) throws TranslateException,
            MathException {
        // ---------------------------------------------
        // 式生成のための前処理
        // ---------------------------------------------
    ...
    protected SyntaxCallFunction createMpiInit() throws MathException {
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynMpiInitCall = new SyntaxCallFunction(
                MPI_FUNC_STR_MPIINIT);

        /* 引数の生成 */
        Math_ci pArgcVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "&" + VAR_STR_ARGC);
        Math_ci pArgvVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "&" + VAR_STR_ARGV);
     ...
     * @author Yuu Shigetani
     */
    protected SyntaxCallFunction createMpiCommSize(Math_ci pMpiCommSizeIndexVar)
            throws MathException {
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynMpiCommSizeCall = new SyntaxCallFunction(
                MPI_FUNC_STR_MPICOMMSIZE);

        /* 引数の生成 */
        Math_ci pArgcVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, MPI_COMM_WORLD);
        pMpiCommSizeIndexVar.setPointerNum(-1);
        Math_ci pArgvVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, pMpiCommSizeIndexVar.toLegalString());
     ...
     * @author Yuu Shigetani
     */
    protected SyntaxCallFunction createMpiCommRank(Math_ci pMpiCommRankIndexVar)
            throws MathException {
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynMpiCommSizeCall = new SyntaxCallFunction(
                MPI_FUNC_STR_COMMRANK);
        ...
        Math_ci pArgvVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, pMpiCommRankIndexVar.toLegalString());
    ...
    protected SyntaxCallFunction createMpiSend(Math_ci pMpiSendData,
        ...
        Math_ci pArgSendData = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, pMpiSendData.toLegalString());
        Math_ci pArgCommWorld = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, MPI_COMM_WORLD);
    ...
    protected SyntaxCallFunction createMpiRecv(Math_ci pMpiRecvData,
        ...
        Math_ci pArgRecvData = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, pMpiRecvData.toLegalString());
        pMpiRecvStatus.setPointerNum(-1);
        Math_ci pArgRecvStatus = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, pMpiRecvStatus.toLegalString());
        Math_ci pArgCommWorld = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, MPI_COMM_WORLD);
