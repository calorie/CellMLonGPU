...
public class CudaProgramGenerator extends ProgramGenerator {
    ...
    // ========================================================
    /*-----プログラム構文取得メソッド-----*/
    public SyntaxProgram getSyntaxProgram() throws CellMLException,
            RelMLException, MathException, TranslateException, SyntaxException {
        /* CellMLにRelMLを適用 */
        ...
        SyntaxPreprocessor pSynInclude3 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_INCLUDE_ABS, "cutil.h");
        SyntaxPreprocessor pSynInclude4 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_INCLUDE_ABS, "cuda.h");
        ...
        SyntaxPreprocessor pSynDefine1 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_DEFINE, CUPROG_DEFINE_DATANUM_NAME + " "
                        + strElementNum);
        ...
        /* メイン関数の生成 */
        CudaMainFuncGenerator pCudaMainFuncGenerator = new CudaMainFuncGenerator(
                m_pCellMLAnalyzer, m_pRelMLAnalyzer, m_pTecMLAnalyzer);
        pCudaMainFuncGenerator.setElementNum(m_unElementNum);
        pCudaMainFuncGenerator.setTimeParam(m_dStartTime, m_dEndTime,
                m_dDeltaTime);
        SyntaxFunction pSynMainFunc = pCudaMainFuncGenerator
                .getSyntaxMainFunction();
        pSynProgram.addFunction(pSynMainFunc);

        /* 初期カーネル生成・追加 */
        CudaInitKernelGenerator pCudaInitKernelGenerator = new CudaInitKernelGenerator(
                m_pCellMLAnalyzer, m_pRelMLAnalyzer, m_pTecMLAnalyzer);
        SyntaxFunction pSynInitKernel = pCudaInitKernelGenerator
                .getSyntaxInitKernel();
        pSynProgram.addFunction(pSynInitKernel);

        /* 計算カーネル生成・追加 */
        CudaCalcKernelGenerator pCudaCalcKernelGenerator = new CudaCalcKernelGenerator(
                m_pCellMLAnalyzer, m_pRelMLAnalyzer, m_pTecMLAnalyzer);
        SyntaxFunction pSynCalcKernel = pCudaCalcKernelGenerator
                .getSyntaxCalcKernel();
    ...
    protected void initialize() throws MathException {
        /* 共通変数生成 */
        m_pKernelIndexVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_KERNEL_INDEX);
        m_pDefinedDataSizeVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_DEFINE_DATANUM_NAME);
    }
    ...
    protected Vector<SyntaxExpression> createExpressions()
        ...
        for (int i = 0; i < nExpressionNum; i++) {
            ...
            if (pLeftExp == null || pRightExp == null) {
                throw new TranslateException("CudaProgramGenerator",
                        "createExpressions", "failed to parse expression");
            }
    ...
    //
    // ========================================================
    protected void replaceFunctionVariables(MathExpression pExpression,
    ...
    protected void addGridDefinitionToProgram(SyntaxProgram pSynDstProgram) {
        ...
        /* define構文の生成 */
        SyntaxPreprocessor pSynDefine1 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_DEFINE, CUPROG_DEFINE_THREADS_SIZE_X_NAME
                        + " " + strThreadsX);
        SyntaxPreprocessor pSynDefine2 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_DEFINE, CUPROG_DEFINE_THREADS_SIZE_Y_NAME
                        + " " + strThreadsY);
        SyntaxPreprocessor pSynDefine3 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_DEFINE, CUPROG_DEFINE_THREADS_SIZE_Z_NAME
                        + " " + strThreadsZ);
        SyntaxPreprocessor pSynDefine4 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_DEFINE, CUPROG_DEFINE_BLOCKS_SIZE_X_NAME
        ...
        SyntaxPreprocessor pSynDefine5 = new SyntaxPreprocessor(
                ePreprocessorKind.PP_DEFINE, CUPROG_DEFINE_BLOCKS_SIZE_Y_NAME
                        + " " + strBlocksY);
    ...
    protected SyntaxFunction createKernel(String strKernelName)
        ...
        SyntaxFunction pSynKernelFunc = new SyntaxFunction(strKernelName,
                pSynVoidType);
        pSynKernelFunc
                .addDeclarationSpecifier(eDeclarationSpecifier.DS_CUDA_GLOBAL);

        /* カーネルインデックス変数の宣言追加 */
        SyntaxDataType SynIntType = new SyntaxDataType(eDataType.DT_UINT, 0);

        Math_ci pTmpIndexVar1 = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_KERNEL_TMP_INDEX1);
        Math_ci pTmpIndexVar2 = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_KERNEL_TMP_INDEX2);
        Math_ci pTmpIndexVar3 = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_KERNEL_TMP_INDEX3);

        SyntaxDeclaration pSynKernelTempIndexVarDec1 = new SyntaxDeclaration(
                SynIntType, pTmpIndexVar1);
        SyntaxDeclaration pSynKernelTempIndexVarDec2 = new SyntaxDeclaration(
                SynIntType, pTmpIndexVar2);
        SyntaxDeclaration pSynKernelTempIndexVarDec3 = new SyntaxDeclaration(
                SynIntType, pTmpIndexVar3);
        SyntaxDeclaration pSynKernelIndexVarDec = new SyntaxDeclaration(
                SynIntType, m_pKernelIndexVar);

        /* 宣言修飾子の追加 */
        pSynKernelTempIndexVarDec1
                .addDeclarationSpecifier(eDeclarationSpecifier.DS_CONST);
        pSynKernelTempIndexVarDec2
                .addDeclarationSpecifier(eDeclarationSpecifier.DS_CONST);
        pSynKernelTempIndexVarDec3
                .addDeclarationSpecifier(eDeclarationSpecifier.DS_CONST);
        pSynKernelIndexVarDec
                .addDeclarationSpecifier(eDeclarationSpecifier.DS_CONST);
        ...
        Math_ci pVarThreadIdxX = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "threadIdx.x");
        Math_ci pVarThreadIdxY = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "threadIdx.y");
        Math_ci pVarBlockIdxX = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "blockIdx.x");
        Math_ci pVarBlockIdxY = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "blockIdx.y");
        Math_ci pVarBlockDimX = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "blockDim.x");
        Math_ci pVarBlockDimY = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "blockDim.y");
        Math_ci pVarGridDimX = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "gridDim.x");
        Math_plus pMathPlus1 = (Math_plus) MathFactory
                .createOperator(eMathOperator.MOP_PLUS);
        Math_plus pMathPlus2 = (Math_plus) MathFactory
                .createOperator(eMathOperator.MOP_PLUS);
        Math_plus pMathPlus3 = (Math_plus) MathFactory
                .createOperator(eMathOperator.MOP_PLUS);
        Math_times pMathTimes1 = (Math_times) MathFactory
                .createOperator(eMathOperator.MOP_TIMES);
        Math_times pMathTimes2 = (Math_times) MathFactory
                .createOperator(eMathOperator.MOP_TIMES);
        Math_times pMathTimes3 = (Math_times) MathFactory
                .createOperator(eMathOperator.MOP_TIMES);
        Math_times pMathTimes4 = (Math_times) MathFactory
                .createOperator(eMathOperator.MOP_TIMES);
    ...
    protected SyntaxDeclaration createBlockDeclaration(Math_ci pBlockVar)
            throws MathException {
        /* 宣言に必要なオペランドの生成 */
        Math_cn pThreadsVarX = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, CUPROG_DEFINE_THREADS_SIZE_X_NAME);
        Math_cn pThreadsVarY = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, CUPROG_DEFINE_THREADS_SIZE_Y_NAME);
        Math_cn pThreadsVarZ = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, CUPROG_DEFINE_THREADS_SIZE_Z_NAME);
        ...
        SyntaxDeclaration pSynBlockVarDec = new SyntaxDeclaration(pSynDim3Type,
                pBlockVar);
    ...
    protected SyntaxDeclaration createGridDeclaration(Math_ci pGridVar)
            throws MathException {
        /* 宣言に必要なオペランドの生成 */
        Math_cn pBlocksVarX = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, CUPROG_DEFINE_BLOCKS_SIZE_X_NAME);
        Math_cn pBlocksVarY = (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, CUPROG_DEFINE_BLOCKS_SIZE_Y_NAME);
        ...
        /* 宣言の生成 */
        SyntaxDataType pSynDim3Type = new SyntaxDataType(eDataType.DT_DIM3, 0);
        SyntaxDeclaration pSynGridVarDec = new SyntaxDeclaration(pSynDim3Type,
                pGridVar);
    ...
    protected SyntaxCallFunction createCudaMalloc(Math_ci pDstVar,
        ...
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynCuMallocCall = new SyntaxCallFunction(
                CUDA_FUNC_STR_CUDAMALLOC);

        /* 第一引数の構築 */
        Math_ci pNewDstVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, "( void** ) &" + pDstVar.toLegalString());
    ...
    protected SyntaxCallFunction createCudaMemcpyH2D(Math_ci pDstVar,
            Math_ci pSrcVar, MathFactor pDataNumFactor) throws MathException {
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynCuMemcpyCall = new SyntaxCallFunction(
                CUDA_FUNC_STR_CUDAMEMCPY);
        ...
        /* 第四引数の構築 */
        Math_ci pFlagVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUDA_CONST_CUDAMEMCPYHOSTTODEVICE);
        // Syntax構文群が完成するまでの暫定処置
    ...
    protected SyntaxCallFunction createCudaMemcpyD2H(Math_ci pDstVar,
        ...
        /* 第四引数の構築 */
        Math_ci pFlagVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUDA_CONST_CUDAMEMCPYDEVICETOHOST);
        // Syntax構文群が完成するまでの暫定処置
    ...
    protected SyntaxCallFunction createCudaMemcpyToSymbol(Math_ci pDstVar,
        ...
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynCuMemcpyCall = new SyntaxCallFunction(
                CUDA_FUNC_STR_CUDAMEMCPYTOSYMBOL);
    ...
    protected SyntaxCallFunction createCudaFree(Math_ci pDstVar) {
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynCuFreeCall = new SyntaxCallFunction(
                CUDA_FUNC_STR_CUDAFREE);
    ...
    //
    // ========================================================
    protected SyntaxCallFunction createCutDeviceInit() throws MathException {
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynCutInitCall = new SyntaxCallFunction(
                CUDA_FUNC_STR_CUTDEVICEINIT);

        /* 引数の生成 */
        Math_ci pArgcVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_VAR_STR_ARGC);
        Math_ci pArgvVar = (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, CUPROG_VAR_STR_ARGV);
    ...
    //
    // ========================================================
    protected SyntaxCallFunction createCutExit() throws MathException {
        /* 関数呼び出しインスタンス生成 */
        SyntaxCallFunction pSynCutExitCall = new SyntaxCallFunction(
                CUDA_FUNC_STR_CUTEXIT);
