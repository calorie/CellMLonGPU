...
public class CommonProgramGenerator extends ProgramGenerator {
    ...
    throws MathException, CellMLException, RelMLException, TranslateException {
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecDerivativeVar().size(); i++) {
            ...
            /*宣言の追加*/
            pSynMainFunc.addStatement(createMalloc(m_pTecMLAnalyzer.getM_vecDerivativeVar().get(i),
                    pMathTimes));
        }
        ...
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecDerivativeVar().size(); i++) {
            ...
            /*宣言の追加*/
            pSynMainFunc.addStatement(createFree(m_pTecMLAnalyzer.getM_vecDerivativeVar().get(i)));
        }
        /*定数のfree*/
        for (int i = 0; i < m_pTecMLAnalyzer.getM_vecConstVar().size(); i++) {
