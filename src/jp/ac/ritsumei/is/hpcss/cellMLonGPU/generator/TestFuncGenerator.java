...
public class TestFuncGenerator extends ProgramGenerator {
    ...
    public SyntaxProgram getSyntaxProgram() throws MathException,
        ...
        SyntaxPreprocessor pSynInclude3 =
            new SyntaxPreprocessor(ePreprocessorKind.PP_INCLUDE_ABS, "sys/stat.h");
    ...
    throws MathException, SyntaxException, TranslateException {
        ...
        Math_ci pDataPlace = createMathCi(createStr(DATA_DIR+SEPARATER+DATA_FILE_NAME+DATA_FILE_EXT).toLegalString());
        Math_ci pStrlenDataPlace = createMathCi(createStrlen(pDataPlace).toLegalStringWithNoSemicolon());
        ...
        //create if
        SyntaxControl pSynIfOpNotNull = createIf(createCondition(pArgvArray, condNull, pMathNeq));
        ...
        //create if
        SyntaxControl pSynIfOpEqualHyphen = createIf(
                createCondition(
                        pArgvWithTwoPointer,
                        createMathCn(createChar("-").toLegalString()),
                        pMathEq));
        ...
        Math_ci pOptionWithOnePointerPlusOnePointer = createMathCi(createPlus(pArgvWithOnePointer, MathCn1).toLegalString());
        ...
        //create if
        SyntaxControl pSynIfOpEqualT= createIf(createCondition(t_pOption, createChar(TEST_MODE), pMathEq));
        ...
        createAssign(
                pSynIfOpEqualT,
                t_pFilePointer,
                createMathCi(createFopen(pTestFileNameVar, createStr("r")).toLegalStringWithNoSemicolon()));
        ...
        //create elseif
        SyntaxControl pSynIfOpEqualW = createElseIf(createCondition(t_pOption, createChar(WRITE_MODE), pMathEq));

        //add if
        pSynIfOpEqualHyphen.addStatement(pSynIfOpEqualW);

        // mkdir
        pSynIfOpEqualW.addStatement(createMkdir(createStr("."+SEPARATER+DATA_DIR)));
        ...
        createAssign(
                pSynIfOpEqualW,
                t_pFilePointer,
                createMathCi(createFopen(pTestFileNameVar, createStr("w")).toLegalStringWithNoSemicolon()));
    ...
    throws MathException, SyntaxException, TranslateException {
        ...
        SyntaxDeclaration pSynArgTestName = createArg(eDataType.DT_CHAR, 1, pTestNameArgVar);

        // arg : douvle cell
        Math_ci pCellArgVar = createMathCi(CELL_NAME);
        SyntaxDeclaration pSynArgCell = createArg(eDataType.DT_DOUBLE, 0, pCellArgVar);

        // arg : douvle diff
        Math_ci pDiffArgVar = createMathCi(DIFF_NAME);
        SyntaxDeclaration pSynArgDiff = createArg(eDataType.DT_DOUBLE, 0, pDiffArgVar);

        // create testCell function
        SyntaxDataType pSynReturnTypeTestInitFunc = new SyntaxDataType(eDataType.DT_VOID, 0);
        SyntaxFunction pSynTestFunc = createFunction(pSynReturnTypeTestInitFunc, TEST_FUNC_NAME, pSynArgTestName, pSynArgCell, pSynArgDiff);
        ...
        createAssign(
                pSynIfOpEqualT,
                t_pHashEntryWithKey,
                createMathCi(createStrdup(pTestNameArgVar).toLegalStringWithNoSemicolon()));

        // ep = hsearch(e, FIND);
        createAssign(
                pSynIfOpEqualT,
                t_pHashEntryPointer,
                createMathCi(createHsearch(t_pHashEntry, "FIND").toLegalStringWithNoSemicolon()));
        ...
        //create if
        SyntaxControl pSynIfEpEqualTrue = createIf(createCondition(t_pHashEntryPointer, MathCnNull, pMathNeq));
...
;
        ...
        // ep = hsearch(e, ENTER);
        createAssign(
                pSynIfEpNotEqualTrue,
                t_pHashEntryPointer,
                createMathCi(createHsearch(t_pHashEntry, "ENTER").toLegalStringWithNoSemicolon()));
        ...
        SyntaxControl pSynWhile = createWhile(
                createCondition(
                        createMathCi(
                                createFgets(
                                        pBuffVarWithNoArrayIndex,
                                        createMathCi(
                                                createSizeof(pBuffVarWithNoArrayIndex).toLegalStringWithNoSemicolon()),
        ...
        pSynWhile.addStatement(createSscanf(pBuffVarWithNoArrayIndex, createStr("%lf%s"), pFCellVarWithPointer, pTagVar));
        ...
        //create if
        SyntaxControl pSynIfTagEqualFilename = createIf(
                createCondition(
                        createMathCi(
                                createStrcmp(pTagVar, pTestNameArgVar).toLegalStringWithNoSemicolon()),
                                MathCn0,
                                pMathEq));
        ...
        //create if
        SyntaxControl pSynIfCntNotEqual0 = createIf(createCondition(pCntVar, MathCn0, pMathNeq));
        ...
        //create if
        SyntaxControl pSynIfAssertCell = createIf(
                createCondition(
                        createMathCi(
                                createFabs(
                                        createMathCi(
                                                createMinus(pCellArgVar, pFCellVar).toLegalString())).toLegalStringWithNoSemicolon()),
                                                pDiffArgVar,
        ...
        // success printf
        pSynIfAssertCell.addStatement(createPrintf(createStr(SUCCESS_MSG), pTestNameArgVar));

        // fail printf
        pSynElseAssertCell.addStatement(createPrintf(createStr(FAIL_MSG), pTestNameArgVar, pCellArgVar, pFCellVar));
        ...
        //create elseif equal w
        //create elseif
        SyntaxControl pSynElseIfOpEqualW = createElseIf(
                createCondition(
                        t_pOption,
                        createChar(WRITE_MODE),
        ...
        // fprintf();
        pSynElseIfOpEqualW.addStatement(
                createFprintfCall(
                        t_pFilePointer,
                        createStr("%lf%s\\n"),
    ...
    public Math_ci createMathCi(String str) throws MathException {
        return (Math_ci) MathFactory.createOperand(
                eMathOperand.MOPD_CI, str);
    }
    public Math_cn createMathCn(String str) throws MathException {
        return (Math_cn) MathFactory.createOperand(
                eMathOperand.MOPD_CN, str);
    }
    ...
    private SyntaxCallFunction createSprintf(Math_ci fileNameBuff, Math_ci node) throws MathException {
        ...
        pSynSprintfCall.addArgFactor(createStr(DATA_DIR+SEPARATER+DATA_FILE_NAME+"%d"+DATA_FILE_EXT));
        pSynSprintfCall.addArgFactor(node);

        return pSynSprintfCall;
    }
    private SyntaxCallFunction createFprintfCall(Math_ci filePointer, Math_cn rowString, Math_ci... args) throws MathException {
    ...
    private SyntaxCallFunction createFopen(Math_ci fileName, Math_cn fileMode) throws MathException {
    ...
    private SyntaxCallFunction createSscanf(Math_ci buff, Math_cn rowString, Math_ci... args) throws MathException {
    ...
    private SyntaxCallFunction createPrintf(Math_cn printString, Math_ci... args) throws MathException {
    ...
    private SyntaxCallFunction createMkdir(MathFactor dir) throws MathException {
        SyntaxCallFunction pSynMkdirCall =
                new SyntaxCallFunction("mkdir");
